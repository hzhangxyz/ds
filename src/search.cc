...
namespace ds {
    ...
    length_t search_t::execute(const std::function<bool(rule_t*)>& callback) {
        ...
        for (auto& rule : temp_rules) {
            auto& movable_rule = const_cast<std::unique_ptr<rule_t>&>(rule);
            rules.emplace(std::move(movable_rule), current_cycle);
        }
        for (auto& fact : temp_facts) {
            auto& movable_fact = const_cast<std::unique_ptr<rule_t>&>(fact);
            facts.emplace(std::move(movable_fact), current_cycle);
        }
